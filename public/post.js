...
function setupFloatingActions() {
    ...
    copyBtn?.addEventListener('click', async () => {
        ...
        setTimeout(() => {
            copyBtn.classList.remove('ok', 'err');
            copyBtn.title = '';
        }, 1200);
    });
    ...
    shareBtn?.addEventListener('click', async () => {
        const res = await shareOrCopy({
            title: qs('#post-title')?.textContent?.trim() || 'Bem Usado',
            text: 'Veja este post do Bem Usado',
            url: location.href
        });
    ...
    // Menu actions
    const reportItem = menu?.querySelector('.k-item.report');
    reportItem?.addEventListener('click', () => {
        // Apenas fecha o menu e permite a navegação para report.html
        closeMenu();
    });
